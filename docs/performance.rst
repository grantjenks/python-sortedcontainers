...
Therefore, for each benchmark below two graphs are shown. The first compares the
performance of sortedcontainers running on CPython 2.7 against competing
implementations. The second displays a performance comparison of
sortedcontainers running on the CPython 2.7, CPython 3.4 and PyPy 2.2
runtimes. In general CPython 3.4 is about half the speed of CPython 2.7. The
PyPy 2.2 runtime displays much more variability due to its JIT-ed nature. Once
the just-in-time compiler optimizes the code, performance is often several to
